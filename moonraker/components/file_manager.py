...
class FileManager:
    ...
    async def _handle_directory_request(self, web_request):
        ...
        async with self.write_mutex:
            result = {'path': directory, 'root': root, 'action': "create_dir"}
    ...
    async def _handle_file_move_copy(self, web_request):
        ...
        async with self.write_mutex:
            ...
            self.notify_sync_lock = None
        result['path'] = self.get_relative_path(dest_root, full_dest)
        return result
        ...
        async with self.write_mutex:
            ...
            try:
                ...
                if e.status_code == 403:
                    raise
            os.remove(full_path)
        return {'path': filename, 'root': root, 'action': "delete_file"}
    ...
    def move_directory_metadata(self, prev_dir, new_dir):
        ...
        for prev_fname in list(self.mddb.keys()):
            if prev_fname.startswith(prev_dir):
                ...
                self.move_file_metadata(prev_fname, new_fname)

    def move_file_metadata(self, prev_fname, new_fname):
